...
\begin{equation}(\#eq:eqn-chainrule-fgh)
\begin{split}
...
| Schritt | Funktionswert | Ableitung | Akkumulation |
| --- | ----- | ------ | ------------ |
| `0a` | $w_1=x_1$ | $\partial w_1=1$ | $\dot w_1 = 1$  |
| `0b` | $w_2=x_2$ | $\partial w_2=0$ | $\dot w_2 = 0$  |
...
| `2` | $w_5 = w_3 + w_4$ | $\partial w_5=\begin{bmatrix}1 & 1\end{bmatrix}$ | $\dot w_5 = \begin{bmatrix}1 & 1\end{bmatrix}\begin{bmatrix} \dot w_3 \\ \dot w_4 \end{bmatrix} = x_1 + \cos(x_1)$ |
...
*totalen Ableitung* einer Funktion $f\colon X\to Y$ bei einem $x\in X$ als die lineare Abbildung $A_x\colon X\to Y$ derart, dass
\begin{equation*}
f(x+h) - f(x) - A_x[h] = o(\| h\|)
\end{equation*}
gilt.
...
F&uuml;r neuronale Netze ist der Gradient von $f(x; \tilde A, b)=Ax+b$ bez&uuml;glich der
...
\end{equation*}
Diese Abbildung ist linear und k&ouml;nnte als Matrix
...
\end{equation*}
und erinnern uns daran, dass f&uuml;r das Training der Gradient vom Ausgang
bez&uuml;glich der Parameter $(A_2, A_1)$ gesucht ist.
...
\end{equation*}
wobei $\sigma'$ passend interpretiert werden muss.
und da die *sp&auml;teren* Parameter (hier $A_2$) nicht von den *fr&uuml;heren*
Parametern (hier $A_1$) abh&auml;ngen
