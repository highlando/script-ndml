...
\leavevmode\vadjust pre{\hypertarget{rem-turing-machine}{}}%
\begin{JHSAYS}
...
\leavevmode\vadjust pre{\hypertarget{rem-algo-annahmen}{}}%
\begin{JHSAYS}
...
\leavevmode\vadjust pre{\hypertarget{rem-algo-kons-stab}{}}%
\begin{JHSAYS}
...
\leavevmode\vadjust pre{\hypertarget{rem-flops}{}}%
\begin{JHSAYS}
...
\BuiltInTok{print}\NormalTok{(}\SpecialStringTok{f\textquotesingle{}x=}\SpecialCharTok{\{}\NormalTok{x}\SpecialCharTok{\}}\SpecialStringTok{: p(x)=}\SpecialCharTok{\{}\NormalTok{cpx}\SpecialCharTok{:.4f\}}\SpecialStringTok{\textquotesingle{}}\NormalTok{)             }\CommentTok{\# Ausgabe des Ergebnisses}
\end{Highlighting}
\end{Shaded}
...
\tightlist
\item
  (\protect\hyperlink{ref-NocW06}{Nocedal and Wright 2006}): Ein gut lesbares Buch zur Optimierung.
\end{itemize}
...
\NormalTok{A\_n }\OperatorTok{=} \OperatorTok{{-}}\DecValTok{1}\OperatorTok{*}\NormalTok{np.diag(np.ones(n}\OperatorTok{{-}}\DecValTok{1}\NormalTok{), }\OperatorTok{{-}}\DecValTok{1}\NormalTok{) }\OperatorTok{+}\NormalTok{ \textbackslash{}  }\CommentTok{\# a tridiagonal band matrix}
...
\BuiltInTok{print}\NormalTok{(}\SpecialStringTok{f\textquotesingle{}n: }\SpecialCharTok{\{}\NormalTok{n}\SpecialCharTok{\}}\SpecialStringTok{ {-}{-} t\_n: }\SpecialCharTok{\{}\NormalTok{toc}\OperatorTok{{-}}\NormalTok{tic}\SpecialCharTok{:.4e\}}\SpecialStringTok{\textquotesingle{}}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\leavevmode\vadjust pre{\hypertarget{rem-accu-iter}{}}%
\begin{JHSAYS}
...
\leavevmode\vadjust pre{\hypertarget{rem-rel-abs-err}{}}%
\begin{JHSAYS}
...
\leavevmode\vadjust pre{\hypertarget{rem-vector-valued-cond}{}}%
\begin{JHSAYS}
...
\leavevmode\vadjust pre{\hypertarget{rem-kons-addition}{}}%
\begin{JHSAYS}
...
\leavevmode\vadjust pre{\hypertarget{rem-conv-iterat}{}}%
\begin{JHSAYS}
...
\begin{proof}
Siehe (\protect\hyperlink{ref-RicW17}{Richter and Wick 2017}, Thm. 6.31)
\end{proof}

\leavevmode\vadjust pre{\hypertarget{rem-smooth-fp-conv}{}}%
\begin{JHSAYS}
...
\leavevmode\vadjust pre{\hypertarget{rem-fpconv-iteration-contraction}{}}%
\begin{JHSAYS}
...
\leavevmode\vadjust pre{\hypertarget{rem-gamma-zerograd}{}}%
\begin{JHSAYS}
...
\item
  Eine schöne Einführung und Übersicht liefert das Skript \emph{Lecture Notes on Iterative Optimization Algorithms} (\protect\hyperlink{ref-Byr14}{Byrne 2014}).
\end{itemize}
...
\leavevmode\vadjust pre{\hypertarget{sdg-put-it-back}{}}%
\begin{JHSAYS}
...
\leavevmode\vadjust pre{\hypertarget{sdg-cnv-independent}{}}%
\begin{JHSAYS}
...
\leavevmode\vadjust pre{\hypertarget{rem-bad-conv-sdg}{}}%
\begin{JHSAYS}
...
\BuiltInTok{print}\NormalTok{(}\SpecialStringTok{f\textquotesingle{}h=}\SpecialCharTok{\{}\NormalTok{h}\SpecialCharTok{:.2e\}}\SpecialStringTok{: diff in gradient }\SpecialCharTok{\{}\NormalTok{hdiff}\SpecialCharTok{.}\NormalTok{flatten()[}\DecValTok{0}\NormalTok{]}\SpecialCharTok{:.2e\}}\SpecialStringTok{\textquotesingle{}}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{    gradnrml.append(}\DecValTok{1}\OperatorTok{/}\NormalTok{batchsize}\OperatorTok{*}\NormalTok{np.linalg.norm(cgrad))}
    \ControlFlowTok{if}\NormalTok{ np.mod(kkk, }\DecValTok{50}\NormalTok{) }\OperatorTok{==} \DecValTok{0}\NormalTok{:}
        \BuiltInTok{print}\NormalTok{(}\SpecialStringTok{f\textquotesingle{}k=}\SpecialCharTok{\{}\NormalTok{kkk}\SpecialCharTok{\}}\SpecialStringTok{: norm of gradient: }\SpecialCharTok{\{}\NormalTok{np}\SpecialCharTok{.}\NormalTok{linalg}\SpecialCharTok{.}\NormalTok{norm(cgrad)}\SpecialCharTok{\}}\SpecialStringTok{\textquotesingle{}}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\item
  Ein Beweis ist in (\protect\hyperlink{ref-BolM04}{Bollhöfer and Mehrmann 2004}, Satz 14.14) zu finden.
\item
...
\end{equation*}
(vgl. Satz 14.15, \protect\hyperlink{ref-BolM04}{Bollhöfer and Mehrmann 2004}).
...
\footnote{denn die Kondition des Eigenwertproblems ist direkt proportional zur Kondition der Matrix; vgl.
  (Satz 5.9, \protect\hyperlink{ref-RicW17}{Richter and Wick 2017})}, da
...
\leavevmode\vadjust pre{\hypertarget{rem-coors}{}}%
\begin{JHSAYS}
...
\leavevmode\vadjust pre{\hypertarget{rem-ortho-bas}{}}%
\begin{JHSAYS}
...
\emph{Tschebyscheff-Approximation}-Problem, sind in (Kapitel 8.7.2, \protect\hyperlink{ref-RicW17}{Richter and Wick 2017}) gut
nachzulesen.
...
\leavevmode\vadjust pre{\hypertarget{ad-vs-sd}{}}%
\begin{JHSAYS}
...
\bottomrule\noalign{}
\endlastfoot
\texttt{0} & \(w_0 = x\) & \(\dot w_0 = 1\) & \(1\) \\
\texttt{1} & \((w_1, \dot w_1) = (h(w_0), h'(w_0))\) & \(\dot w_1\) & \(\dot w_1 \cdot 1\) \\
\texttt{2} & \((w_2, \dot w_2) = (g(w_1), g(w_1))\) & \(\dot w_2\) & \(\dot w_2 \cdot\dot w_1 \cdot 1\) \\
\texttt{3} & \((w_3, \dot w_3) = (f(w_1), f(w_1))\) & \(\dot w_3\) & \(\dot w_3 \cdot\dot w_2 \cdot\dot w_1 \cdot 1\) \\
\end{longtable}
...
\leavevmode\vadjust pre{\hypertarget{fwd-ad-vs-bwd}{}}%
\begin{JHSAYS}
...
\bottomrule\noalign{}
\endlastfoot
\texttt{0a} & \(w_1=x_1\) & & \(\dot w_1 = 1\) \\
\texttt{0b} & \(w_2=x_2\) & & \(\dot w_2 = 0\) \\
\texttt{1a} & \(w_3 = w_1w_2\) & \(\partial w_3 = \begin{bmatrix} w_2&w_1\end{bmatrix}\) & \(\dot w_3 = \begin{bmatrix}w_2&w_1\end{bmatrix}\begin{bmatrix} 1 \\ 0 \end{bmatrix} = x_2\) \\
\texttt{1b} & \(w_4 = \sin(w_1)\) & \(\partial w_4=\cos(w_1)\) & \(\dot w_4 =\cos(w_1)\cdot\dot w_1 = \cos(x_1)\) \\
\texttt{2} & \(w_5 = w_3 + w_4\) & \(\partial w_5=\begin{bmatrix}1 & 1\end{bmatrix}\) & \(\dot w_5 = \begin{bmatrix}1 & 1\end{bmatrix}\begin{bmatrix} \dot w_3 \\ \dot w_4 \end{bmatrix} = x_2 + \cos(x_1)\) \\
\end{longtable}
...
\bottomrule\noalign{}
\endlastfoot
\texttt{0} & \(y=w_5\) & \(\partial y=1\) & \(\bar w_5 = 1\) \\
\texttt{1} & \(w_5=w_4+w_3\) & \(\partial w_5=\begin{bmatrix} 1&1 \end{bmatrix}\) & \(\bar w_5 = 1\) \\
\texttt{2a} & \(w_4=\sin(w_1)\) & \(\partial w_4=\cos(w_1)\) & \(\bar w_4 = \bar w_5 \cdot 1 = 1\) \\
\texttt{2b} & \(w_3=w_1w_2\) & \(\partial w_3=\begin{bmatrix}w_2&w_1\end{bmatrix}\) & \(\bar w_3 = \bar w_5 \cdot 1 = 1\) \\
\texttt{3b} & \(w_2=x_2\) & & \(\bar w_2 = \bar w_3 w_1= x_1\) \\
\texttt{3a} & \(w_1=x_1\) & & \(\bar w_1 = \bar w_3 w_2 +\bar w_4 \cos(w_1) = x_2 + \cos(x_1)\) \\
\end{longtable}
...
\end{equation*}
definiert ist (siehe beispielsweise (\protect\hyperlink{ref-KurZ05}{Kurdila and Zabarankin 2005}, Theorem 5.5.1) für ein
abstraktes Resultat und bspw. (\protect\hyperlink{ref-Tr09}{Tröltzsch 2009}, Abschnitt 5.9.1) für eine
...
\leavevmode\vadjust pre{\hypertarget{Lx-not-LA}{}}%
\begin{JHSAYS}
...
\leavevmode\vadjust pre{\hypertarget{grad-vs-jac}{}}%
\begin{JHSAYS}
...
Für die Ableitung bezüglich \(A_k\) in einem \(N\)-layer Netzwerk bekommen
...
\hypertarget{implementierungen-und-beispiele}{%
\section{Implementierungen und Beispiele}\label{implementierungen-und-beispiele}}

Jan könnte als grobe Einschätzung sagen:

\leavevmode\vadjust pre{\hypertarget{ad-applications}{}}%
\begin{JHSAYS}
Die Eleganz und Effizienz von AD für Optimierungsanwendungen ist
...
\emph{nonstandard but clever} Funktionen können mit entsprechendem Aufwand
...
Sicherlich ist es am besten im Bezug auf Aufwand und Funtionalität, den
code sofort mit Augenmerk auf AD zu entwickeln.

Soll der AD \emph{overhead} nicht selbst mitentwickelt werden, empfiehlt es sich auf
Programmumgebungen zurückzugreifen, die einen \emph{AD framework} mitbringen.
...
Für bestehenden Code gibt es zwei Herangehensweisen (neben der
``Um''entwicklung).

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\item
  \emph{precompiling} -- ``AD ready'' Code wird automatisiert aus bestehendem Code
  generiert.
\item
  \emph{overloading} -- Es werden AD Variablentypen und Klassen definiert und die
  Grundoperationen \emph{überladen} sodass mit minimalen Änderungen ein
  bestehender Code die AD Funktionalität bekommt.
\end{enumerate}
...
\tightlist
\item
  \href{https://doi.org/10.1007/s11081-019-09474-x}{Algorithmic differentiation of an industrial airfoil design tool coupled with the adjoint CFD method} -- ein Beispiel in dem ein kompletter Simulationscode \emph{automatisch differenziert} wurde um Tragflügel zu optimieren.
\end{itemize}

\hypertarget{implementierungen}{%
\subsection{Implementierungen}\label{implementierungen}}

\begin{itemize}
\tightlist
\item
  \href{https://web.casadi.org/}{CASADI} -- eine \emph{AD aware} (für \texttt{python} und \texttt{Matlab/Octave}) Umgebung zur Optimierung (auch mit
  Differentialgleichungen)
\item
  \href{https://pytorch.org/}{PyTorch} -- eine \emph{machine learning toolbox} mit AD integriert zur
  Gradientenberechnung
\item
  \href{https://www3.math.tu-berlin.de/Vorlesungen/SS06/AlgoDiff/adolc-110.pdf}{ADOL-C} -- ein \emph{overloading} framework für \texttt{C} und \texttt{C++}
\item
  \href{https://tapenade.gitlabpages.inria.fr/userdoc/build/html/tapenade/whatisad.html}{Tapenade} -- ein \emph{precompiler} der aus Code \emph{AD Code} generiert
\end{itemize}
...
\leavevmode\vadjust pre{\hypertarget{ref-BolM04}{}}%
Bollhöfer, M., Mehrmann, V.: \href{https://doi.org/10.1007/978-3-322-80242-2}{Numerische {Mathematik}. {Eine} projektorientierte {Einf{ü}hrung} f{ü}r {Ingenieure}, {Mathematiker} und {Naturwissenschaftler}.} Vieweg (2004)

\leavevmode\vadjust pre{\hypertarget{ref-Byr14}{}}%
Byrne, C.L.: Lecture notes on iterative optimization algorithms, \url{https://faculty.uml.edu/cbyrne/IOIPNotesOct2014.pdf}, (2014)

\leavevmode\vadjust pre{\hypertarget{ref-KurZ05}{}}%
Kurdila, A.J., Zabarankin, M.: Convex functional analysis. {Birkhäuser} (2005)

\leavevmode\vadjust pre{\hypertarget{ref-NocW06}{}}%
Nocedal, J., Wright, S.J.: \href{https://doi.org/10.1007/978-0-387-40065-5}{Numerical optimization}. Springer (2006)

\leavevmode\vadjust pre{\hypertarget{ref-RicW17}{}}%
Richter, T., Wick, T.: \href{https://doi.org/10.1007/978-3-662-54178-4}{Einf{ü}hrung in die numerische {Mathematik}. {Begriffe}, {Konzepte} und zahlreiche {Anwendungsbeispiele}}. Heidelberg: Springer Spektrum (2017)

\leavevmode\vadjust pre{\hypertarget{ref-Tr09}{}}%
