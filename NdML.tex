...
\end{equation*}
für eine Funktion \(g\colon \mathbb R^{n} \to \mathbb R^{}\), also die Aufgabe ein \(x^*\in \mathbb R^{n}\) zu finden, für welches der Wert von \(g\) minimal wird.

\leavevmode\hypertarget{rem-why-g}{}%
\begin{JHSAYS}
Es wird gleich wieder darum gehen, eine Nullstelle zu finden (nämlich die des Gradienten von \(g\)). Um uns eine beliebte Schwierigkeit zu ersparen (beziehungsweise den Gedankenschritt, dass die Nullstelle nicht für \(f\) sondern für \(\nabla f\) gesucht wird), nennen wir die Funktion hier lieber \(g\).

\end{JHSAYS}

Ist \(g\) differenzierbar (der Einfachheit halber nehmen wir an, dass \emph{totale} Differenzierbarkeit vorliegt; es würde aber Differenzierbarkeit in einer beliebigen Richtung, also \emph{Gateaux}-Differenzierbarkeit, genügen), so gilt, dass in einem Punkt \(x_0\), der Gradient \(\nabla g(x_0)\) (ein Vektor im \(\mathbb R^{n}\)) in die Richtung des stärksten Wachstums zeigt und der negative Gradient \(-\nabla g(x_0)\) in die Richtung, in der \(g\) kleiner wird.
...
\begin{JHSAYS}
Was ist wenn \(\nabla g(x_0) = 0\) ist und warum gibt es andernfalls so ein \(\gamma_0\) und wie könnten wir es systematisch bestimmen?
...
\protect\hypertarget{def:def-grad-descent}{}\label{def:def-grad-descent}Sei \(g\colon \mathbb R^{n} \to \mathbb R^{}\) differenzierbar, dann heißt die Iteration
\begin{equation}
x_{k+1} := x_k - \gamma_k\nabla g(x_k)
...
\protect\hypertarget{lem:lem-graddesc-as-fp}{}\label{lem:lem-graddesc-as-fp}Sei \(D\subset \mathbb R^{n}\) offen und der Definitionsbereich von \(g\), ist \(x^*\in D\) ein Minimum von \(g\) und ist \(\nabla g\colon D \to \mathbb R^{n}\) \emph{Lipschitz-stetig} mit Konstante \(L\), dann definiert \eqref{eq:eqn-grad-desc} mit \(\gamma_k \equiv \frac L2\) eine konvergente Fixpunktiteration für \(\phi(x) = x-\gamma \nabla g(x)\) mit \(x^*\) als Fixpunkt.
\end{lemma}
...
\item
  Beweisen Sie, dass für \(0<\gamma< \frac{2}{\|A^TA\|_2}\) gilt, dass\(\|I-\gamma A^TA\|<1\) für beliebige \(A\in \mathbb R^{m \times n}\).
\item
