...
\item
  ...
  y_i\bigl ( x_i-b, \, w\bigr)  >0, \quad i=1,\dotsc,N
  \end{equation}
\item
...
\item
  ...
  \(\bigl ( x-b, \, w\bigr) \) entscheiden, auf \emph{welcher Seite} von \(H\) ein Punkt \(x\) liegt.
...
\item
  ...
  Vorzeichen von \(\bigl ( x-b, \, w\bigr) \) gelabelt werden -- das ist die eigentliche
  Motivation, diese Hyperebene möglichst gut zu bestimmen.
\item
  Es gilt \(\bigl ( x-b, \, w\bigr) =\bigl ( x, \, w\bigr) -\bigl ( b, \, w\bigr) :=\bigl ( x, \, w\bigr)  -\beta\). Und damit
...
dass
\begin{equation*}
h_x = \|x\|_2\cos(\phi(w,x))=\|x\|_2 \frac{\bigl ( w, \, x\bigr)  }{\|w\|_2\|x\|_2} = \frac{\bigl ( w, \, x\bigr) }{\|w\|_2}
\end{equation*}
wobei
\(\cos(\phi(w, x))\)
...
\begin{equation*}
\bigl ( x-b, \, w\bigr)  = \bigl ( x-\frac{h}{\|w\|_2} w, \, w\bigr)  = \bigl ( x, \, w\bigr)  - h\|w\|_2
= \|w\|_2 (\frac{\bigl ( x, \, w\bigr)  }{\|w\|_2} - h) = \|w\|_2(h_x - h)
\end{equation*}
den mit \(\|w\|_2\) skalierten Abstand (inklusive dem Vorzeichen) enthält,
beziehungsweise, dass der Abstand als
\begin{equation*}
y_i\frac{\bigl ( x_i-b, \, w\bigr) }{\|w\|_2} = y_i\frac{\bigl ( x_i, \, w\bigr) -\beta}{\|w\|_2}
\end{equation*}
...
\begin{equation*}
\max_{w\in \mathbb R^{n}, \, \beta \in \mathbb R^{}} \min_{x_i \in \mathbb X} y_i\frac{\bigl ( x_i, \, w\bigr) -\beta}{\|w\|_2}
\end{equation*}
formuliert werden.
...
\tightlist
\item
  Sei die Hyperebene über \(w\) und \(b\) gegeben. Bestimmen Sie den Abstand
