...
\item
  ...
  Stetige Funktion \(f_N\) mit endlich vielen \emph{Sprungstellen}, sodass
  \begin{equation*}
...
In Schritt 1, wird die stückweise stetige Funktion \(f_N\) definiert. Die
Existenz folgt aus dem Satz

\begin{theorem}[Stückweise stetige Funktion liegen dicht]
\protect\hypertarget{thm:thm-pl-dense-C}{}\label{thm:thm-pl-dense-C}Sei \([a, b]\subset \mathbb R^{}\) ein abgeschlossenes endliches Intervall.
Die Menge \(\operatorname{pl}[a, b]\) aller stückweise stetigen Funktionen auf \([a, b]\) \emph{liegt dicht}
in \(\mathcal C[a, b]\) sodass für ein beliebiges \(f\in \mathcal C[a, b]\) und \(\varepsilon > 0\), immer ein \(g\in \operatorname{pl}[a, b]\) existiert mit \(\|f-g\|_\infty<\varepsilon\).
\end{theorem}

\begin{proof}
Der Beweis ist klassisch -- hier nur die relevanten und konstruktiven Elemente.

An sich muss für Dichtheit gezeigt werden, dass es zu jeder Funktion \(f\in \mathcal C[a, b]\) als eine Folge in \(\operatorname{PL}[a, b]\) gibt, mit \(f\) als Grenzwert. Wir
...
\label{eq:eqn-fj-fjp-se}, dass auch die punktweise Differenz kleiner als \(\varepsilon\)
ist.
