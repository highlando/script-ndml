...
\end{equation*}
für eine Funktion \(f\colon \mathbb R^{n} \to \mathbb R^{}\), also die Aufgabe ein \(x^*\in \mathbb R^{n}\) zu finden, für welches der Wert von \(f\) minimal wird.
...
Die vorhergegangenen Überlegungen gingen von \(x^*\) innerhalb eines offenen Definitionsbereichs \(D\) von \(f\) aus, wo ein Minimum durch \(\nabla f(x^*)\) und \(f(x^*)\leq f(x)\) für alle \(x\) aus einer Umgebung von \(x^*\).
Ein typischer Anwendungsfall ist, dass \(x^*\) in einem zulässigen Bereich \(C\) liegen muss, der eine echte Teilmenge von \(D\) ist.
...
\protect\hypertarget{thm:thm-prj-grad-desc}{}\label{thm:thm-prj-grad-desc}bla
\end{theorem}
